...
# Verificar e criar a tabela Circuitos se não existir
cur.execute('''
...
''')
print("Tabela 'Circuitos' verificada/criada com sucesso.")
...
def circuito_existe(email_circuito):
    try:
        SELECT = f"SELECT COUNT(*) FROM Circuitos WHERE email_circuito = '{email_circuito}'"
...
def insertCirc(id_cliente, email_circuito, descricao, status):
    try:
        ...
        if not existe:
            INSERT = f"""
            INSERT INTO Circuitos(id_cliente,email_circuito,descricao,status) VALUES
